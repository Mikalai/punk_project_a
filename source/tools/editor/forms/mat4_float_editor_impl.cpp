...
namespace Tools {
	...
	void Mat4FloatEditorImpl::On15Changed(wxCommandEvent& event)
	{
		...
		if (m_15->GetValue().ToDouble(&v)) {
			m_value->at(15) = (float)v;
		}
	}

	void Mat4FloatEditorImpl::SetSourceValue(const Core::String& name, Math::mat4* value) {
		m_value = value;
