...
{
	for(unsigned i = 0; i < faceInfos.size(); ++i)
	{
...
bool NvStripInfo::Unique(NvFaceInfoVec& faceVec, NvFaceInfo* face)
{
	...
	for(unsigned i = 0; i < faceVec.size(); i++)
	{
		if(!bv0)
		{
...
void NvStripInfo::Build(NvEdgeInfoVec &edgeInfos, NvFaceInfoVec &faceInfos)
{
	...
	for(unsigned i = 0; i < forwardFaces.size(); i++)
...
void NvStripifier::RemoveSmallStrips(NvStripInfoVec& allStrips, NvStripInfoVec& allBigStrips, NvFaceInfoVec& faceList)
{
	...
	for(unsigned i = 0; i < allStrips.size(); i++)
	{
		if(int(allStrips[i]->m_faces.size()) < minStripLength)
		{
			...
			for(unsigned j = 0; j < allStrips[i]->m_faces.size(); j++)
	...
	if(tempFaceList.size())
	{
		...
		while(1)
		{
			...
			for(unsigned i = 0; i < tempFaceList.size(); i++)
			{
				if(bVisitedList[i])
					continue;
							...
							NvStripInfoVec &outStrips, NvFaceInfoVec& outFaceList)
{
	...
	cacheSize = std::max<int>(1, in_cacheSize - CACHE_INEFFICIENCY);
	...
	for (unsigned i = 0; i < allEdgeInfos.size(); i++)
	{
...
//    outStrips.push_back(tempStrips[i]);
	...
	if(tempStrips2.size() != 0)
	{
		...
		for(unsigned i = 0; i < tempStrips2.size(); i++)
		{
            int numNeighbors = 0;

			//find strip with least number of neighbors per face
			for(unsigned j = 0; j < tempStrips2[i]->m_faces.size(); j++)
			{
...
{
	for(unsigned i = 0; i < strip->m_faces.size(); ++i)
	{
		...
		for (int i = 0; i < numExperiments; i++)
		{
			if (i != bestIndex)
			{
				...
				for (int j = 0; j < numStrips; j++)
				{
					...
					for (unsigned k = 0; k < currStrip->m_faces.size(); ++k)
					{
