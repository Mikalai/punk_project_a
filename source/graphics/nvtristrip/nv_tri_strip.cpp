...
void Cleanup(NvStripInfoVec& tempStrips, NvFaceInfoVec& tempFaces)
{
	...
	{
		for(unsigned j = 0; j < tempStrips[i]->m_faces.size(); j++)
		{
					...
					PrimitiveGroup** primGroups, unsigned short* numGroups, bool validateEnabled)
{
	...
	if (validateEnabled)
	{
		...
		for (unsigned i = 0; i < in_numIndices; i += 3)
		{
		...
		for (unsigned i = 0; i < *numGroups; ++i)
		{
			switch ((*primGroups)[i].type)
			{
				case PT_LIST:
				{
					for (unsigned j = 0; j < (*primGroups)[i].numIndices; j += 3)
					{
				...
				case PT_STRIP:
				{
					int brokenCtr = 0;
					bool flip = false;
					for (unsigned j = 2; j < (*primGroups)[i].numIndices; ++j)
					{
