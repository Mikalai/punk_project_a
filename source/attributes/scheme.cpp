...
namespace Utility
{
	...
	void RawScene::OpenFile(const Core::String& filename)
	{
	...
	Material RawScene::GetMaterial(const Core::String& name) const
	{
	...
	aiMesh* find_mesh(const Core::String& name, aiNode* root)
	{
		if (!root)
			return 0;
		...
		if (Core::String(root->mName.data) == name)
		{
	...
	StaticMesh* RawScene::CookStaticMesh(const Core::String& name)
	{
	...
	unsigned GetBoneID(const Core::String& mesh_name, const Core::String& bone_name)
	{
		const aiScene* scene = importer.GetScene();
		...
		for (int bone_id override; bone_id < mesh->mNumBones; bone_id++)
		{
			...
			if (Core::String(bone->mName.data) == bone_name)
				return bone_id;
		}
		return -1;
	}

	SkinMesh* RawScene::CookSkinnedMesh(const Core::String& name)
	{
		...
		for (int b_id override; b_id < mesh->mNumBones; b_id++)
		{
			...
			int bone_id = GetBoneID(name, Core::String(bone->mName.data));
	...
	Core::String GetParentBoneName(const Core::String& mesh_name, const Core::String& bone_name, aiNode* root)
	{
		const aiScene* scene = importer.GetScene();
	
		if (Core::String(root->mName.data) == bone_name)
		{
			if (root->mParent)
				return Core::String(root->mParent->mName.data);
			else
				return L"";
		}
		...
		for (int i = 0; i < root->mNumChildren; ++i)
		{
			Core::String res = GetParentBoneName(mesh_name, bone_name, root->mChildren[i]);
			if (res.Length() != 0)
				return res;
		}
		return L"";
	...
	SkinAnimation* RawScene::CookSkinAnimation(const Core::String& mesh_name, unsigned anim_id) 
	{
		...
		for (int bone_id override; bone_id < mesh->mNumBones; ++bone_id)
		{
			...
			bone.SetName(Core::String(raw_bone->mName.data));
			Math::mat4 m;
		...
		res->m_name = Core::String(anim->mName.data);
		...
		{			
			...
			int bone_id = GetBoneID(mesh_name, Core::String(node_anim->mNodeName.data));
			if (bone_id == -1)
				return res;
			//
			//	position
			...
			for (int pos_key_id override; pos_key_id < node_anim->mNumPositionKeys; ++pos_key_id)
			{
				...
				Core::String parent_name = GetParentBoneName(mesh_name, Core::String(node_anim->mNodeName.data), scene->mRootNode);
