...
namespace Attributes
{
	...
	bool Terrain::AddOrUpdateCell(const TerrainCell& value)
	{
		if (value.GetLocation().X() < 0 || value.GetLocation().X() >= m_cells.ColumnCount())
            throw System::Error::SystemException(L"Cell X position " + Core::String::Convert(value.GetLocation().X()) + L" is incorrect. Should be in the range [0; " + Core::String::Convert(m_cells.ColumnCount()) + L"]");
		if (value.GetLocation().Y() < 0 || value.GetLocation().Y() >= m_cells.RowCount())
            throw System::Error::SystemException(L"Cell Y position " + Core::String::Convert(value.GetLocation().Y()) + L" is incorrect. Should be in the range [0; " + Core::String::Convert(m_cells.RowCount()) + L"]");
	...
	const Core::String Terrain::ToString() const
	{
