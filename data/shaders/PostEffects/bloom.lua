...
function Script:Render(camera,context,buffer,depth,diffuse,normals,emission)
	...
	if self.buffer~=nil then
			if self.buffer[0]:GetWidth()~=self.w/2 or self.buffer[0]:GetHeight()~=self.h/2 then
	...
	if self.buffer==nil then
			...
			self.buffer[0]=Buffer:Create(self.w/4,self.h/2,1,0)
			self.buffer[1]=Buffer:Create(self.w/8,self.h/4,1,0)
